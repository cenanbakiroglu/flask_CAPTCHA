...
from captcha import captcha, captchatext
...
failed_attempts = {}
captcha_flag=False #captcha bayrağı
...
def captchatextget(): # captcha kodunun yolu
    return captchatext()
...
def login():
    ...
    if form.validate_on_submit():   # CSRF ve diğer validasyonları otomatik kontrol eder
        ...
        message="İstenilen bilgiler hatalı!"
        if username in failed_attempts:
            ...
            if attempts > 2 :
                ...
                captcha_flag = True
                form.kod.validators = [DataRequired()]  # Burada kod alanını zorunlu yapıyoruz
                form.kod.render_kw = {'required': 'required'}  # HTML tarafında da gerekli olduğunu belirtiyoruz     
        ...
        if user and check_password_hash(user["password"], password) and captcha_flag==False or user and check_password_hash(user["password"], password) and captcha_flag and form.kod.data==captchatextget():  # Hash doğrulama
        ...
        return render_template('login.html', form=form, captcha_flag=captcha_flag,  message=message) # captcha bayrağını ve message değişkeninin son halini HTML tarafına gönderiyoruz
...
if __name__ == '__main__':
    ...
    # formun onload kısmı eklenerek sayyfa her yeni yüklendiğinde CHAPTCHAnin yenilenmesini sağla x
    # form da CHAPTCHA kısmını hidden olarak ayarla sonra js kullanarak belirli deneme sayısından sonra chaptcha'yi aktifleştir
    # CHAPTCHA session içinde kullanma x
